...
resource "azurerm_resource_group" "test_group" {
  name     = "${module.naming.resource_group.slug}-${module.naming.storage_managed_encryption_key.slug}-max-test-${local.unique_name_stub}"
  location = "uksouth"
}
...
module "terraform-azurerm-storage-managed-encryption-key" {
  ...
  client_key_permissions  = ["get", "create", "unwrapkey", "wrapkey"]
  storage_key_permissions = ["get", "create", "unwrapkey", "wrapkey"]
  key_type                = "RSA"
  key_length              = 4096
  key_options             = []
}
